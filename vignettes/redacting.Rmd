...
<!-- TODO: rewrite using a set-cookie response header -->
...
Sensitive or personal information is not limited to response cookies or headers. Sometimes personal identifiers are built into URLs or response bodies. These may be less sensitive than auth tokens, but you probably want to conceal or anonymize your data that is included in test fixtures.
...
We'd rather not have that information leak in our test fixtures, so in the [pivotaltrackR](http://enpiar.com/r/pivotaltrackR/) package, which wraps this API, we need to tell `capture_requests` to scrub this id when we record mocks.
...
Note the formula shorthand: this follows the syntax in the [`purrr`](purrr.tidyverse.org) package for defining anonymous functions. It is equivalent to `function (response) gsub_response(response, getOption("pivotal.project"), "123")`.
...
We can do more response cleaning with custom functions. All of the redactors in `httptest` take the "response" object as their first argument and return the response object modified in some way. This lends them to pipelining, as with the [`magrittr`](http://magrittr.tidyverse.org/) package.
