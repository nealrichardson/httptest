...
So, in the [pivotaltrackR](https://github.com/nealrichardson/pivotaltrackR) package, which wraps this API, if we want to record mocks to use in tests with this API, we need to tell `capture_requests` to scrub its special header. To do this, we'll use `set_redactor()` to supply a custom function.
