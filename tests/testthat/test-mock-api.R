public({
  with_mock_api({
    ...
    test_that("GET files that don't exist errors", {
      ...
      expect_GET(
        GET("api/NOTAFILE/", query = list(a = 1)),
        "api/NOTAFILE/?a=1"
      )
    })
    ...
    test_that("max.print option", {
      ...
      expect_PUT(
        PUT("http://httpbin.org/get", body = '{"test":true}'),
    ...
    test_that("Request body and query", {
      expect_PATCH(
        PATCH("api/object2?d=1", body = '{"arg":45}'),
    ...
    test_that("Request preprocessing via set_requester: change the request URL", {
      ...
      expect_identical(
        content(GET("http://pythong.org/get")),
        content(g1)
      )
    })
  })
